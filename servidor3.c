...
void passTurn(grupo_de_sockets *sockets);
...
int main(int argc, char *argv[]){
	...
	player2 = &(sockets.client_info[1]);
	player1->logged = player2->logged = false;
	player1->named = player2->named = false;
	////////////////////////////////////////////////////////////////////////////
	while(1){
		...
		if((player1->logged && player2->logged) && (player1->named && player2->named)){
			printf("\n\n[SERVIDOR] > 2 clientes conectados, iniciamos partida\n");
			// START <name> <rows> <cols> le tenemos que pasar a cada jugador el nombre del jugador CONTRARIO
			//printf("\n[SERVIDOR] -> START %s %d %d\n", player2->username, board.rows, board.cols);
			//printf("\n[SERVIDOR] -> START %s %d %d\n", player1->username, board.rows, board.cols);
		
			fprintf(player1->stream, "START %s %d %d\n", player2->username, board.rows, board.cols);
			fprintf(player2->stream, "START %s %d %d\n", player1->username, board.rows, board.cols);
			int first = rand()%2; //elegimos un jugador entre los 2 para que comience la partida
			//printf("\n[SERVIDOR] -> Comienza la partida el jugador %d", first);
			if (first == 1){
				player1->symbol = 'o';
				player2->symbol = 'x';
				player1->turn = player2->turn = 'o';
	...
	if(strcmp(option, "COLUMN") == 0){
		...
		if(boardState(board, player)){
			...
			if(endGame(*board, player->symbol)){
				...
				if(sockets->client_info[0].symbol != player->symbol)
...
void passTurn(grupo_de_sockets *sockets){
	...
	player1 = &(sockets->client_info[0]);
	player2 = &(sockets->client_info[1]);
	//printf("\n[SERVIDOR] <TURNO> -> player1 turn: %c, symbol: %c", player1->turn, player1->symbol);
	//printf("\n[SERVIDOR] <TURNO> -> player2 turn: %c, symbol: %c", player2->turn, player2->symbol);
	if(player1->turn == player1->symbol ){
		player1->turn = player2->symbol;
		player2->turn = player2->symbol;
		fprintf(player2->stream, "URTURN %d\n", player1->lastMove);
	}
	else{
		player2->turn = player1->symbol;
		player1->turn = player1->symbol;
		fprintf(player1->stream, "URTURN %d\n", player2->lastMove);	
	}
	//printf("\n[SERVIDOR] <TURNO> -> player1 turn: %c, symbol: %c", player1->turn, player1->symbol);
	//printf("\n[SERVIDOR] <TURNO> -> player2 turn: %c, symbol: %c", player2->turn, player2->symbol);
}
